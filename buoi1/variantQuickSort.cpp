...
void variantQuickSort(recordType R[], int l, int r) {
  int pivot = getPivot(R, l, r);
  if (pivot == -1)
    return;
  int point = _partition(R, l, r, pivot);
  variantQuickSort(R, l, point - 1); // [l; point - 1]
  variantQuickSort(R, point, r);     // [point; r]
}
...
int main() {
  ...
  printf("After using variantQuickSort:\n");
  for (int i = 0; i < n; i++) {
    printf("%d - %.2f\n", R[i].key, R[i].otherFields);
  }
  variantQuickSort(R, 0, n - 1);
  printf("Before using variantQuickSort:\n");
  for (int i = 0; i < n; i++) {
    printf("%d - %.2f\n", R[i].key, R[i].otherFields);
  }
  return 0;
