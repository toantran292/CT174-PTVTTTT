...
void in_ma_tran(canh a[][size], int n) {
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++)
            printf(" %c%c =%5.2f  ", a[i][j].dau + 97, a[i][j].cuoi + 97,
                   a[i][j].do_dai);
        printf("\n");
    }
}
...
void Cap_Nhat_PA_TNTT(canh a[][size], int n, float TGT, float *GNNTT, canh x[],
                      canh PA[]) {
    int i;
    // mang x da co n-1 canh tu x[0] den x[n-2]
    x[n - 1] = a[x[n - 2].cuoi][x[0].dau];  // Noi dinh cuoi cua x[n-2] voi dinh
                                            // dau cua x[0] de co chu trinh
    TGT = TGT + x[n - 1].do_dai;  // Cong them vao TGT do dai cua canh noi dinh
                                  // cuoi voi dinh dau
    if (*GNNTT > TGT) {
        *GNNTT = TGT;
...
// Neu ta di toi dinh ke_tiep ma dinh ke_tiep nay trung voi mot dinh dau cua mot
// canh nao do thi tao thanh chu trinh
...
void Nhanh_Can(canh a[][size], int n, int i, int dau, float *TGT, float *CD,
               float *GNNTT, canh x[], canh PA[]) {
        ...
        if (dau != j && !a[dau][j].da_dung && !co_chu_trinh(x, i, j)) {
            ...
            *CD = can_duoi(a, *TGT, n, i + 1);
            if (*CD < *GNNTT) {  // Neu CD >= GNNTT thi cat tia
                ...
                if (i == n - 2) {  // Da xet het tat ca cac dinh nhung duong di
                                   // moi co n-1 canh
                    Cap_Nhat_PA_TNTT(a, n, *TGT, GNNTT, x, PA);
                } else
                    Nhanh_Can(a, n, i + 1, j, TGT, CD, GNNTT, x,
                              PA);  // Xet nut con cua nut i
            }
